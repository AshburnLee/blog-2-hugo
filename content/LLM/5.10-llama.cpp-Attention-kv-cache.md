...
2. **当前 token 先计算 Q、K、V，追加 K、V 到 KV-cache，再计算注意力**。然后用当前 Q 与整个 KV-cache（包括新 K 和 V）计算注意力。
3. 推理完成或上下文重置时，清空 KV-cache

注：

- 上述的 “计算所有 token 的”，所有指的是输入prompt 经过tokenize 后的token 和 在自回归过程中已生成的 token
- 一般会对 kv-cache 进行量化。
